...
COLUMNS_SOURCE = ["FILE","DATE","TARGET","FOIL","CURRENT_MAX", "CURRENT_MIN","CURRENT_AVE","CURRENT_STD","VOLTAGE_MAX","VOLTAGE_MIN","VOLTAGE_AVE","VOLTAGE_STD","HFLOW_AVE","HFLOW_STD", "RATIO_MIN","RATIO_AVE","RATIO_STD","SOURCE_STABILITY_AVE","SOURCE_STABILITY_STD"]
...
COLUMNS_BEAM = ["FILE","DATE","TARGET","FOIL","COLL_CURRENT_L_MAX","COLL_CURRENT_L_MIN","COLL_CURRENT_L_AVE","COLL_CURRENT_L_STD","COLL_CURRENT_R_MAX","COLL_CURRENT_R_MIN","COLL_CURRENT_R_AVE","COLL_CURRENT_R_STD"
    ...
    "PROBE_MAX","PROBE_MIN","PROBE_AVE","PROBE_STD"]
...
"RF":[['DEE1_VOLTAGE','DEE2_VOLTAGE'],["FORWARD_POWER"],["REFLECTED_POWER_W"],["FLAP1","FLAP2"],['PHASE_LOAD']],
...
"BEAM":[[[120,130,150]],[[]],[[110,120,130]],[[20,25,35]],[[-0.5,-1,-2]],[[0.5,2,10]]],
"VACUUM":[[[1.8,1.9,2,"Upper limit"]],[[60,50,40,"Lower limit"]],[[500,700,800,"Upper limit"]],[[2,5,7.5,"Upper limit"]],[[0.35,0.4,0.5,"Upper limit"]]],
"RF":[[[]],[[13.5,14.5,15,"Upper limit"]],[[400,500,600,"Upper limit"]],[[10,5,3,"Lower limit"]],[[5.5,6,6.5,"Upper limit"],[4.5,4,3.5,"Lower limit"]]],
"RF_STABILITY":[[[]],[[2,2.5,10,"Upper limit"]],[[1,-0.5,-2.5,"Lower limit"]],[[7.5,10,12.5,"Upper limit"]],[[7,10,15,"Upper limit"]]],
...
"BEAM":[[r"I foil [uA]"], ["I target [\u03bcA]"],["I probe [\u03bcA]"],["I collimators[%]"],["I target stability [%]"],["Extraction losses [%]"]],          # he cambiado I probe por I target
...
def initial_df(self):               
        ...
        self.df_summary_source = pd.DataFrame(columns=[COLUMNS_SOURCE])
        self.df_summary_vacuum = pd.DataFrame(columns=[COLUMNS_VACUUM])
        self.df_summary_beam   = pd.DataFrame(columns=[COLUMNS_BEAM])
        self.df_summary_rf     = pd.DataFrame(columns=[COLUMNS_RF])
        self.df_summary_transmission = pd.DataFrame(columns=[COLUMNS_TRANSMISSION])
        self.df_extraction_target    = pd.DataFrame(columns=[COLUMNS_EXTRACTION])
        self.volume_information      = pd.DataFrame(columns=[COLUMNS_FILLING])
        self.df_source_performance   = pd.DataFrame(columns=["FILE","TARGET","SOURCE_PERFORMANCE_AVE","SOURCE_PERFORMANCE_STD","TRANSMISSION"])
        self.volume_information      = pd.DataFrame(columns=[COLUMNS_VOLUME])
