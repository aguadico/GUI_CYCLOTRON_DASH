...
def get_summary_vacuum(self):
    ...
    ave_vacuum,std_vacuum,max_vacuum,min_vacuum = getting_subsystems_data.get_statistic_values(vacuum_level)
    ...
    float(std_vacuum)/float(ave_vacuum)*100,0,float(ave_vacuum)/float(ave_gas_flow)*1e5,((float(std_vacuum)**2+float(std_gas_flow)**2))**0.5/float(ave_gas_flow)*1e5]]
...
def get_summary_volume(self):
    ...
    ave_pressure,std_pressure,max_pressure,min_pressure = getting_subsystems_data.get_statistic_values(pressure_irradiation)
    ave_volume,std_volume,max_volume,min_volume = getting_subsystems_data.get_statistic_values(volume)
...
def get_summary_magnet(self):
    ...
    ave_magnet_current,std_magnet_current,max_magnet_current,min_magnet_current = getting_subsystems_data.get_statistic_values(magnet_current)
    ...
    magnet_current = pd.concat([self.time_smaller_current,self.magnet_current_total],axis=1,keys=["Time","Current_I"])
...
def get_summary_extraction(self):
    ...
    ave_carousel_position,std_carousel_position, max_carousel_position, min_carousel_position = getting_subsystems_data.get_statistic_values(carousel_position)
    ave_balance_position,std_balance_position, max_balance_position, min_balance_position = getting_subsystems_data.get_statistic_values(balance_position)
    print ("TARGET NUMBER")
...
def get_summary_beam(self):
    ...
    max_probe,min_probe,ave_probe,std_probe]]
