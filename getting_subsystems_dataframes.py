...
def get_subsystems_dataframe_source(self):
    ...
    source_voltage,source_current,gas_flow = getting_subsystems_data.get_source_parameters(self.file_df,self.max_current)
...
def get_subsystems_dataframe_vacuum(self):
    ...
    vacuum_level = getting_subsystems_data.get_vacuum_parameters(self.file_df,self.max_current)
    magnet_current = getting_subsystems_data.get_magnet_parameters(self.file_df,self.max_current)
...
def get_subsystems_dataframe_magnet(self):    
    ...
    self.magnet_current = getting_subsystems_data.get_magnet_parameters(self.file_df,self.max_current)
    self.magnet_current_total = getting_subsystems_data.get_magnet_parameters(self.file_df,15)
...
def get_subsystems_dataframe_rf(self):
    ...
    dee1_voltage,dee2_voltage = getting_subsystems_data.get_rf_parameters(self.file_df,self.max_current) 
    forwarded_power,reflected_power,phase_load = getting_subsystems_data.get_rf_parameters_power(self.file_df,self.max_current)
    flap1_pos,flap2_pos = getting_subsystems_data.get_rf_parameters_flaps(self.file_df,self.max_current)
...
def get_subsystems_dataframe_rf_sparks(self):
    ...
    source_voltage,source_current,gas_flow = getting_subsystems_data.get_source_parameters(self.file_df,-1)
    dee1_voltage,dee2_voltage = getting_subsystems_data.get_rf_parameters(self.file_df,self.low_source_current) 
    forwarded_power,reflected_power,phase_load = getting_subsystems_data.get_rf_parameters_power(self.file_df,self.low_source_current)
    flap1_pos,flap2_pos = getting_subsystems_data.get_rf_parameters_flaps(self.file_df,self.low_source_current)
...
def get_subsystems_dataframe_extraction(self):
    ...
    carousel_position,balance_position = getting_subsystems_data.get_extraction_parameters_position(self.file_df,self.max_current)
...
def get_subsystems_dataframe_beam(self):
    df_column_names_beam = ["Time","Foil_No","Foil_I","Coll_l_I","Target_I","Coll_r_I","Coll_l_rel","Coll_r_rel","Target_rel","Extraction_losses","Probe_I"]
    collimator_r,collimator_l = getting_subsystems_data.get_collimator_parameters(self.file_df,self.max_current) 
    extraction_current = getting_subsystems_data.get_extraction_parameters(self.file_df,self.max_current)
    ...
    probe = getting_subsystems_data.get_probe_current_irradiation(self.file_df,self.max_current)
    df_subsystem_values_beam = [self.time,self.foil_number,extraction_current,collimator_l,self.target_current,collimator_r,collimator_l_rel,collimator_r_rel,target_rel,extraction_losses,probe]
...
def get_subsystems_dataframe_pressure(self):
    ...
    target_pressure = getting_subsystems_data.get_target_pressure(self.file_df,self.max_current)
...
def get_subsystems_dataframe_pressure_irradiation(self):
    df_column_names_pressure = ["Time","Foil_No","Target_P"]
    target_pressure = getting_subsystems_data.get_target_pressure_irradiation(self.file_df,self.max_current)
    df_subsystem_values_pressure = [self.time,self.foil_number,target_pressure]
